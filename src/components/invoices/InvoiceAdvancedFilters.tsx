...
export function InvoiceAdvancedFilters({
  ...
  const [local, setLocal] = useState<InvoiceFilters>({});
      ...
      <SheetContent className="w-[380px] sm:w-[420px] overflow-y-auto">
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            <div className="space-y-2">
              <Label className="text-sm font-medium">Invoice Amount Greater Than</Label>
              <Input
                type="number"
          ...
          <div className="space-y-2">
            ...
            <Input
              placeholder="e.g. CAM-202601"
              value={local.campaign_contains || ""}
              onChange={(e) =>
                setLocal({ ...local, campaign_contains: e.target.value || undefined })
              }
              className="h-9"
            />
          </div>
...
/** Active filter pills for invoices */
export function InvoiceFilterPills({
  filters,
  onClear,
  onClearAll,
}: {
  filters: InvoiceFilters;
  onClear: (key: keyof InvoiceFilters) => void;
  onClearAll: () => void;
}) {
