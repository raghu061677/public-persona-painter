...
import {
  ALL_ASSET_EXPORT_FIELDS,
  DEFAULT_ASSET_CUSTOM_FIELDS,
  ASSET_FIELD_GROUPS,
  generateMediaAssetsCustomExcel,
} from "@/lib/reports/generateMediaAssetsCustomExcel";
import { generateMediaAssetsCustomPpt } from "@/lib/reports/generateMediaAssetsCustomPpt";
import { useToast } from "@/hooks/use-toast";

interface MediaAssetsCustomExportDialogProps {
  open: boolean;
...
export function MediaAssetsCustomExportDialog({
  open, onOpenChange, rows, companyName, themeColor,
}: MediaAssetsCustomExportDialogProps) {
  const { toast } = useToast();
  const [selectedFields, setSelectedFields] = useState<string[]>([...DEFAULT_ASSET_CUSTOM_FIELDS]);
  ...
  const toggleGroup = (group: string) => {
    const groupKeys = ALL_ASSET_EXPORT_FIELDS.filter(f => f.group === group).map(f => f.key);
  ...
  const selectAll = () => setSelectedFields(ALL_ASSET_EXPORT_FIELDS.map(f => f.key));
  const resetToDefault = () => setSelectedFields([...DEFAULT_ASSET_CUSTOM_FIELDS]);
  ...
  const handleExport = async (type: "excel" | "ppt") => {
    ...
    try {
      if (type === "excel") {
        await generateMediaAssetsCustomExcel(rows, selectedFields, companyName);
      } else {
        await generateMediaAssetsCustomPpt(rows, selectedFields, companyName, themeColor);
      }
      toast({
        title: "Export Complete",
        description: `${type === "excel" ? "Excel" : "PPT"} exported with ${selectedFields.length} columns and ${rows.length} assets`,
      });
      onOpenChange(false);
    } catch (err) {
      console.error("Custom export error:", err);
      toast({ title: "Export Failed", description: `Could not generate ${type === "excel" ? "Excel" : "PPT"}`, variant: "destructive" });
    } finally {
      setExporting(null);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Select the fields to include. {rows.length} assets will be exported based on current filters.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <Badge variant="secondary" className="ml-auto">
            {selectedFields.length} / {ALL_ASSET_EXPORT_FIELDS.length} fields
          </Badge>
        </div>
        ...
        <ScrollArea className="h-[360px] pr-4">
          <div className="space-y-4">
            {ASSET_FIELD_GROUPS.map(group => {
              const groupFields = ALL_ASSET_EXPORT_FIELDS.filter(f => f.group === group);
              return (
                <div key={group}>
                  <div className="flex items-center gap-2 mb-2">
                    <Checkbox
