...
import type { PaymentFilters } from "./PaymentAdvancedFilters";

interface PaymentQuickChipsProps {
...
const STATUS_CHIPS: { label: string; value: string[] | undefined }[] = [
  ...
  { label: "Unpaid", value: ["Sent", "Partial", "Overdue", "Pending"] },
  { label: "Paid", value: ["Paid"] },
];

export function PaymentQuickChips({
  filters,
  onFiltersChange,
  onOpenAdvanced,
}: PaymentQuickChipsProps) {
  const now = new Date();
  ...
  const handleDueSoon = () => {
    ...
    onFiltersChange({
      ...filters,
      status: ["Sent", "Partial", "Overdue", "Pending"],
      due_between: { from, to },
      duration_days: 7,
      month: undefined,
    });
  ...
  const isDueSoon = filters.duration_days === 7 && filters.status?.includes("Sent");
    ...
    <div className="space-y-2 mb-4">
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        {STATUS_CHIPS.map((chip) => (
          <button
            key={chip.label}
            onClick={() => onFiltersChange({ ...filters, status: chip.value })}
            className={chipClass(isStatusActive(chip.value))}
          >
            {chip.label}
          </button>
        ))}
        <button onClick={handleDueSoon} className={chipClass(isDueSoon)}>
          Due Soon
        </button>
      </div>
