...
function renderRevenueTrendTable(doc: jsPDF, data: { month: string; revenue: number; expenses: number; profit: number }[], startY: number, marginLeft: number, contentWidth: number, p: Palette): number {
  ...
  autoTable(doc, {
    ...
    body: tableData,
    theme: 'plain',
    styles: { font: 'NotoSans', fontSize: 7, cellPadding: 2, textColor: [...p.dark] as [number, number, number] },
    headStyles: { fillColor: [...p.muted] as [number, number, number], textColor: [...p.dark] as [number, number, number], fontStyle: 'bold' as const },
