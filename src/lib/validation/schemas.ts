...
export const campaignEntitySchema = z.object({
  ...
  status: z.enum(["Draft", "Upcoming", "Running", "Completed", "Cancelled", "Archived"]).default("Draft"),
  notes: z.string().max(2000).optional().or(z.literal("")),
}).refine(data => {
...
export const mediaAssetEntitySchema = z.object({
  ...
  media_type: z.string().trim().min(2).max(100),
  dimensions: z.string().trim().min(3).max(50),
  card_rate: z.number().min(0, "Card rate must be non-negative"),
