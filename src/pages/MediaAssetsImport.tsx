      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          const insertData = {
            ...
            illumination_type: row.illumination || null,
            total_sqft: totalSqft,
            is_multi_face: row.isMultiFace || false,
            card_rate: row.cardRate ? parseFloat(row.cardRate) : 0,
            base_rate: row.baseRent ? parseFloat(row.baseRent) : null,
            gst_percent: row.gstPercent ? parseFloat(row.gstPercent) : 18,
            printing_rate_default: row.printingCharges ? parseFloat(row.printingCharges) : null,
            mounting_rate_default: row.mountingCharges ? parseFloat(row.mountingCharges) : null,
