...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Generate invoice ID
    const { data: invoiceId } = await supabaseClient.rpc('generate_invoice_id');

    // Calculate invoice items from campaign assets
    const items = campaign.campaign_assets.map((asset: any) => ({
      description: `${asset.media_type} - ${asset.location}`,
      rate: asset.card_rate,
      printing_charges: asset.printing_charges || 0,
      mounting_charges: asset.mounting_charges || 0,
      total: asset.card_rate + (asset.printing_charges || 0) + (asset.mounting_charges || 0)
    }));
    ...
    console.log(`Invoice ${invoiceId} auto-generated for campaign ${campaign_id}`);
