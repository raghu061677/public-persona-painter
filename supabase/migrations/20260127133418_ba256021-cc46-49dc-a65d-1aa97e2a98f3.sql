...
-- Update soft_delete_campaign to also release media assets
...
BEGIN
  ...
  -- NEW: Release media assets that were booked for this campaign
  -- Only release if no other active campaigns reference them
  WITH released AS (
    UPDATE media_assets
    SET 
    ...
    AND NOT EXISTS (
      ...
      WHERE ca.asset_id = media_assets.id
        ...
        AND c.status IN ('Draft', 'Upcoming', 'Running', 'Planned')
