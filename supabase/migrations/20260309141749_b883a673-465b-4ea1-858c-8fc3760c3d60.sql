...
-- Fix get_next_code_number to check existing plans/campaigns when initializing counter
...
BEGIN
  ...
  ELSIF p_counter_type = 'CLIENT' THEN
    ...
    v_next_value := v_max_existing + 1;
  ELSIF p_counter_type = 'PLAN' THEN
    -- Check existing plans table for max sequence in this period
    SELECT COALESCE(MAX(
      CASE 
        WHEN id ~ '-[0-9]+$' 
        THEN CAST(SUBSTRING(id FROM '[0-9]+$') AS integer)
    ...
    INTO v_max_existing
    FROM plans
    WHERE id LIKE 'PLAN-' || p_period || '-%';
    
    v_next_value := v_max_existing + 1;
  ELSIF p_counter_type = 'CAMPAIGN' THEN
    -- Check existing campaigns table for max sequence in this period
    SELECT COALESCE(MAX(
      CASE 
        WHEN id ~ '-[0-9]+$' 
        THEN CAST(SUBSTRING(id FROM '[0-9]+$') AS integer)
    ...
    INTO v_max_existing
    FROM campaigns
    WHERE id LIKE 'CAM-' || p_period || '-%';
    
    v_next_value := v_max_existing + 1;
  ELSIF p_counter_type = 'INVOICE' THEN
    SELECT COALESCE(MAX(
      CASE 
        WHEN id ~ '-[0-9]+$' 
        THEN CAST(SUBSTRING(id FROM '[0-9]+$') AS integer)
  ...
  DO UPDATE SET 
    current_value = GREATEST(code_counters.current_value + 1, EXCLUDED.current_value),
    updated_at = now()
