CREATE OR REPLACE FUNCTION public.can_reuse_cancelled_invoice_number(p_invoice_id text)
RETURNS boolean
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  IF to_regclass('public.payment_records') IS NOT NULL THEN
    EXECUTE 'SELECT EXISTS (SELECT 1 FROM public.payment_records WHERE invoice_id = $1 LIMIT 1)'
  ...
  IF to_regclass('public.payment_confirmations') IS NOT NULL THEN
    EXECUTE 'SELECT EXISTS (SELECT 1 FROM public.payment_confirmations WHERE invoice_id = $1 LIMIT 1)'
    INTO v_has_dependency
    USING p_invoice_id;
    ...
    IF NOT EXISTS (
      ...
      WHERE id = v_candidate
        AND (
          ...
          OR NOT public.can_reuse_cancelled_invoice_number(v_candidate)
        )
    ) THEN
      RETURN v_candidate;
    END IF;
...
BEGIN
  ...
  WHERE id = v_new_id
    ...
    AND public.can_reuse_cancelled_invoice_number(v_new_id);
