-- Dedupe duplicate auto-seeded campaign_charge_items caused by a race in lazy seeding.
-- Step 1: For campaigns that have month-assigned seeded rows, remove uninvoiced cycle-seeded rows
--         (those should have been migrated to billing_month_key).
...
-- Step 2: Within the same (campaign, asset, charge_type, billing slot), keep one row.
WITH ranked AS (
  SELECT
    id,
    is_invoiced,
